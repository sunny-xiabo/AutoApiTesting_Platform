...
TestData = CaseData(ConfigHandler.data_path + r'CLTEmissionScource/clt_get_EmissionSource.yaml').case_process()
re_data = regular(str(TestData))


@allure.epic("Carbon Lens TEC")
@allure.feature("排放源管理模块")
class TestCltGetEmissionsource:

    @allure.story("排放源列表接口")
    @pytest.mark.parametrize('in_data', eval(re_data), ids=[i['detail'] for i in TestData])
    def test_clt_get_EmissionSource(self, in_data, case_skip):
        """
        :param :
        :return:
        """
...
if __name__ == '__main__':
    pytest.main(['test_clt_get_EmissionSource.py', '-s', '-W', 'ignore:Module already imported:pytest.PytestWarning'])
